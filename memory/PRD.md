...
1. **UI: Сортировка по бренду + цене** (`OfferSelectModal.js`)
   - Сначала альтернативы с тем же брендом (по цене ↑)
   - Затем остальные бренды (по цене ↑)
   - Tie-breaker: supplier_name → id

2. **Backend: Tail State б/х vs с/х** (`npc_matching_v9.py`)
   - Добавлены паттерны: `б/х`, `с/х`, `безхв`, `на хвосте`
   - Gate применяется ТОЛЬКО если REF имеет tail_state
   - REF tail_on → кандидат без tail_state → REJECT (TAIL_STATE_UNKNOWN)

3. **Backend: Northern Species** (`npc_matching_v9.py`)
   - `northern`: северн, ботан, pandalus, coldwater, норвеж, гренланд
   - Species hard gate отделяет northern от vannamei/tiger

4. **UI: Debug скрыт по умолчанию** (`OfferSelectModal.js`)
   - Debug banner показывается только при `?debug=1` или `localStorage.DEBUG_NPC=1`

**Acceptance Criteria:**
- ✅ ref_caliber != null → strict содержит ТОЛЬКО тот же калибр
- ✅ REF б/х ≠ кандидат с/х (TAIL_STATE_MISMATCH)
- ✅ species=northern ≠ vannamei
- ✅ Debug не виден по умолчанию
- ✅ Сортировка: бренд → цена
- ✅ 334 теста прошли

---

### Phase 26 - P0: Расширенное SHRIMP Detection (контекстное) — 28 января 2026

**ЗАДАЧА:** "ванамей б/г 16/20" без слова "креветки" не распознавался как SHRIMP → strict=0.

**РЕШЕНИЕ:**

1. **Расширенное SHRIMP определение:**
   - `SHRIMP_TERMS`: ваннам, vannamei, тигров, королевск, северн, аргентин, лангустин...
   - `SHRIMP_ATTRS`: б/г, с/г, очищ, с/м, в/п, шт/ф, iqf, block, во льду...
   - `detect_shrimp_by_context()`: если калибр + ≥2 SHRIMP_ATTRS → SHRIMP

2. **Порядок проверок в `_detect_npc_domain`:**
   ```
   1. check_blacklist() → if True → return None (FORBIDDEN_CLASS)
   2. SHRIMP_EXCLUDES (со вкусом, чипс, сухар)
   3. SHRIMP_TERMS (явные)
   4. detect_shrimp_by_context() (калибр + атрибуты)
   5. SEAFOOD, FISH, MEAT
   ```

3. **FORBIDDEN_CLASS теперь блокирует domain:**
   - "Гёдза с креветкой 16/20" → `npc_domain=None` (раньше было SHRIMP)
   - "Пельмени с креветкой" → `npc_domain=None`

4. **Новые поля в ref_debug:**
   - `is_shrimp_by_context`: bool
   - `shrimp_attr_count`: int
   - `why_empty_strict`: string

**Acceptance Criteria:**
- ✅ "ванамей б/г 16/20" → `npc_domain=SHRIMP`, `ref_caliber=16/20`
- ✅ "очищ б/г 26/30 с/м block" → `npc_domain=SHRIMP` (контекст)
- ✅ "Гёдза с креветкой 16/20" → `npc_domain=None` (blacklisted)
- ✅ 334 теста прошли
...
1. **Backend API (`routes.py`):**
   - Добавлено поле `strict_after_gates` — ЕДИНСТВЕННЫЙ источник данных для UI в strict режиме
   - Добавлен `debug_id` (UUID) для трассировки каждого запроса
   - Логирование: `[debug_id] item_id=X ref_caliber=Y strict_count=Z rejected={...}`

2. **Frontend (`OfferSelectModal.js`):**
   - UI рендерит ТОЛЬКО `strict_after_gates` (не `alternatives`, не `similar`)
   - Дополнительная валидация калибра на клиенте (belt-and-suspenders)
   - Console.log: `debug_id, ref_caliber, strict_after_gates.length, rejected_reasons`
   - Если strict пустой → "Нет сопоставимых предложений"

3. **Hard Gates (SHRIMP):**
   - `CALIBER_MISMATCH` — 16/20 ≠ 31/40/26/30/21/25
   - `CALIBER_MISSING` — REF имеет калибр, кандидат нет → REJECT
   - `FORBIDDEN_CLASS` — гёдза/пельмени/суп → REJECT
   - `UOM_MISMATCH` — шт ≠ кг
...
{
  "debug_id": "abc123",
  "ref_parsed": {"shrimp_caliber": "16/20"},
  "strict_after_gates": [...],
  "rejected_reasons": {"CALIBER_MISMATCH": 3, "FORBIDDEN_CLASS": 2}
}
```

**Acceptance Criteria:**
- ✅ В strict НЕТ 31/40, 26/30, 21/25
- ✅ В strict НЕТ гёдза/пельмени/суп
- ✅ В strict ТОЛЬКО совпадающий калибр
- ✅ 334 теста прошли
