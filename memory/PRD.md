...
1. `short_tokens` обрезал слова до 4 символов ("кури", "филе") и находил частичные совпадения
...
2. **Добавлены word boundaries (`\b`)** в regex — теперь ищем только целые слова
3. **Увеличен минимум символов для синонимов** — с 3 до 4 символов
4. **Приоритет поиска**:
   - `lemma_tokens $all` — основной, самый точный (морфологический)
   - `synonym_regex` с word boundaries — для синонимов
   - `exact tokens` с word boundaries — точные слова

**Результат**:
- "куриное филе": 14 товаров (было 358!) — только релевантные
- "масло сливочное": 28 товаров — все масла/продукты с маслом
- "лосось филе": 17 товаров — только лосось

**Файлы изменены**:
- `/app/backend/search_synonyms.py` — build_synonym_regex с word boundaries
- `/app/backend/bestprice_v12/routes.py` — удалён short_tokens, добавлены \b
...
- По запросу пользователя удалён компонент `SearchAutocomplete` из страницы каталога
...
- **Проблема**: модалка показывала все товары с тем же `product_core_id` (например, горбушу вместе с лососем)
- **Решение**: переписан алгоритм поиска альтернатив:
  1. Сначала ищем по ТОЧНОМУ `name_norm` — идентичные товары от разных поставщиков
  2. Если не найдено, ищем по `product_core_id` + похожее название (первые 2 значимых слова)
- **Результат**: модалка теперь показывает только действительно идентичные товары
- Файл: `routes.py` → endpoint `/item/{item_id}/alternatives`

### ✅ Phase 9 - Рефакторинг, Autocomplete, Индексы (20 января 2026)
