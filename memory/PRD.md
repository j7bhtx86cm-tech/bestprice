...
### Phase 18 - NPC Matching v9.1 "Нулевой мусор" — 26 января 2026

**Задача (P0)**: Реализация принципа "ничего лишнего" в matching

#### Описание:
NPC v9.1 обеспечивает **нулевой мусор** в результатах matching. Strict показывает **только точные аналоги** или пустой список. Similar доступен **только по кнопке**.

#### РЕЖИМЫ ВЫДАЧИ:
- `mode=strict` (по умолчанию): только точные аналоги, Similar=[]
- `mode=similar` (по кнопке): Strict + Similar с лейблами

#### HARD-ПРАВИЛА (Strict 1-в-1):

**1. PROCESSING_FORM:**
- `CANNED` (ж/б, консервы) ≠ `SMOKED` (х/к, г/к) ≠ `FROZEN_RAW` (с/м) ≠ `CHILLED_RAW` (охл)
- ✅ "Скумбрия в масле ж/б" → НЕ "Скумбрия х/к"

**2. CUT_TYPE:**
- `WHOLE_TUSHKA` (тушка) ≠ `FILLET` (филе) ≠ `STEAK` (стейк) ≠ `MINCED` (фарш)
- ✅ "Окунь тушка с/м" → НЕ "Окунь филе с/м"

**3. SPECIES:**
- Строгое совпадение вида: окунь ≠ сибас ≠ скумбрия
- ✅ "Окунь тушка" → НЕ "Сибас тушка"

**4. IS_BOX:**
- Короб исключается если REF не короб
- ✅ "Лосось 1кг" → НЕ "Лосось 10кг/кор"

**5. SHRIMP (строго 1-в-1):**
- species: vannamei ≠ tiger ≠ argentine
- state: raw_frozen ≠ cooked_frozen
- form: peeled ≠ shell_on, headless ≠ head_on
- caliber: 16/20 ≠ 21/25 ≠ 31/40
- ✅ "Креветки 16/20" → НЕ "Креветки 31/40"

**6. FISH SIZE:**
- Размер/граммовка с допуском 30%
- ✅ "Окунь 300-500г" → НЕ "Окунь 90-150г"

#### РАНЖИРОВАНИЕ Strict:
1. Близость размера/калибра (size_score)
2. Совпадение бренда
3. ppu_value
4. min_line_total
...
#### API:
```
GET /api/v12/item/{item_id}/alternatives?mode=strict  # По умолчанию
GET /api/v12/item/{item_id}/alternatives?mode=similar # По кнопке
```
...
{
  "source": {
    "processing_form": "FROZEN_RAW",
    "cut_type": "FILLET",
    "species": "salmon",
    "is_box": false
  },
  "strict": [...],
  "similar": [],  // Пусто в mode=strict
  "strict_count": 5,
  "similar_count": 0,
  "rejected_reasons": {"SPECIES_MISMATCH": 15, "CUT_TYPE_MISMATCH": 8}
}
```
