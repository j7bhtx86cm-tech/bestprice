...
## Текущий статус: ✅ PHASE 3 - CART OPTIMIZER (2026-01-19)

---

### LATEST: Phase 3 - Cart → Checkout Optimizer (2026-01-19) ✅

**ТЗ выполнено:**

**1. Минималки поставщиков (настраиваемые в БД):**
- Интегрита — 7 000₽
- Айфрут — 5 000₽
- РБД — 3 500₽
- Остальные — 10 000₽
...
**2. Состояния корзины:**
- **Draft (raw)**: "намерения" клиента, qty = requested, нет замен/оптимизаций
- **Plan (optimized)**: назначены supplier_item_id + supplier_id, флаги изменений

**3. Матчинг позиции:**
- ✅ Жёсткие условия: active=true, price>0, product_core_id, unit_type
- ✅ Фасовка ±20%
- ✅ Критические атрибуты (fat_pct, cut) если заданы
- ✅ Brand preference с fallback

**4. Оптимизация:**
- ✅ +10% topup — ТОЛЬКО увеличение qty существующих позиций
- ✅ Если минималка не достигнута → перераспределение на других поставщиков
- ✅ Если все под минималкой → показываем blocked_reason

**5. UI на подтверждении:**
- ✅ Назначенный поставщик
- ✅ Флаги: "Поставщик изменён", "Бренд заменён", "Фасовка ±20%", "+10% для минималки"
- ✅ Строка "Запрошено: X → Будет заказано: Y"

**Файлы:**
- `/app/backend/bestprice_v12/optimizer.py` — полностью переписан
- `/app/frontend/src/pages/customer/CustomerCart.js` — two-phase UI

**Acceptance Criteria:**
1. ✅ До "Оформить заказ" — корзина НЕ меняет товары/qty
2. ✅ На "Оформить заказ" — распределяет по поставщикам, применяет +10%
3. ✅ +10% — ТОЛЬКО увеличением qty существующих позиций
4. ✅ Если минималка не достигнута — blocked_reason
5. ✅ Клиент видит маркировки замен и "Запрошено → Будет заказано"

---

### Exact Add-to-Cart Fix (2026-01-19) ✅

**Проблема:**
После GOLD миграции qty менялся при добавлении (1 → 2), цена/бренд/поставщик прыгали.

**Решение - Two-Phase Cart:**
1. **Raw Cart View (до checkout)**: показывает ТОЧНО что пользователь добавил
   - qty = qty_requested (без округлений)
   - цена = цена товара (без перерасчётов)
   - НЕТ бейджей замены
   
2. **Optimized View (после "Оформить заказ")**: показывает оптимизированный план
   - qty может измениться (минималки, округления)
   - бейджи замены показываются
   - пользователь может вернуться "← Назад в корзину"

**Файлы изменены:**
- `/app/frontend/src/pages/customer/CustomerCart.js` - полностью переписан с two-phase UI
- `/app/backend/bestprice_v12/routes.py` - update/delete по supplier_item_id

**Acceptance Criteria:**
1. ✅ Добавили qty=1 → в корзине qty=1
2. ✅ ID товара совпадает
3. ✅ Цена не прыгает до checkout
4. ✅ Бейджи показываются только на экране подтверждения

---

### Fix Add-to-Cart After GOLD Migration (2026-01-19) ✅
...
**Реализовано:**
- ✅ **Скрипт миграции**: `/app/backend/gold_migration.py`
- ✅ **8 поставщиков мигрированы** на GOLD прайс-листы
- ✅ **Старые данные деактивированы** (active=false)
- ✅ **Маркеры batch_id** для отслеживания версий данных

**Результаты миграции:**

| Поставщик | До | После | Batch Marker |
|-----------|---:|------:|--------------|
| Айфрут | 779 | 778 | GOLD_20260119_111100 |
| Алиди | 1587 | 1586 | GOLD_20260119_111100 |
| Интегрита | 1984 | 1974 | GOLD_20260119_111101 |
| ПраймФудс | 29 | 29 | GOLD_20260119_111103 |
| РБД | 434 | 433 | GOLD_20260119_111103 |
| Сладкая жизнь | 217 | 217 | GOLD_20260119_111103 |
| Ромакс | 1310 | 1265 | GOLD_20260119_111103 |
| Восток-Запад | 928 | 883 | GOLD_20260119_111104 |
| Нордико | 627 | 625 | GOLD_20260119_112549 |
| **ИТОГО** | **7895** | **7790** | |

**Примечания:**
- ✅ Все 9 поставщиков мигрированы на GOLD прайс-листы
- Все GOLD-записи помечены флагом `is_gold: true`
- API каталога протестирован и работает корректно

**Файлы GOLD прайс-листов:**
```
/app/backend/gold_pricelists/
├── GOLD_Aifruit.xlsx      # 780 строк
├── GOLD_Alidi.xlsx        # 1587 строк  
├── GOLD_Integrita.xlsx    # 2025 строк
├── GOLD_PrimeFoods.xlsx   # 30 строк
├── GOLD_RBD.xlsx          # 435 строк
├── GOLD_Romanov.xlsx      # 218 строк (→ Сладкая жизнь)
├── GOLD_Romax.xlsx        # 1268 строк
└── GOLD_VZ.xlsx           # 895 строк
```

---

### Offer Validation Gate - Hard Gate для качества данных (2026-01-16) ✅
