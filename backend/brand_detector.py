    ...
    for word in words:
        if word.isupper() and len(word) > 3:
            ...
            excluded = [
                ...
                'РЫБА', 'МЯСО', 'ОВОЩИ', 'ФИЛЕ', 'СТЕЙК', 'ТУШКА',
                'СУХАРИ', 'ХЛЕБ', 'БУЛКА', 'БАТОН',  # Хлебобулочные
                'РИС', 'ГРЕЧКА', 'ПШЕНО', 'МАНКА',  # Крупы
                'САХАР', 'СОЛЬ', 'ПЕРЕЦ', 'УКСУС',  # Приправы базовые
                'МОЛОКО', 'СМЕТАНА', 'ТВОРОГ', 'КЕФИР',  # Молочка базовая
            ]
