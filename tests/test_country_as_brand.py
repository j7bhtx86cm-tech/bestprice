...
class TestCountryAsBrand:
    ...
    def test_argentina_country_triggers_country_as_brand(self, auth_headers):
        ...
        # Verify selected item has origin_country='АРГЕНТИНА'
        selected = data.get("selected_item", {})
        selected_country = (selected.get("origin_country") or "").upper()
        assert selected_country == "АРГЕНТИНА", \
            f"Expected selected item origin_country='АРГЕНТИНА', got '{selected_country}'"
