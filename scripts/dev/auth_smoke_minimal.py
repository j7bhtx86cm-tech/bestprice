#!/usr/bin/env python3
"""Minimal auth smoke: GET /docs then POST login for integrita, romax, gmfuel. Stdout: 3 lines. Details in artifacts/auth_smoke_minimal.log."""
import json
import sys
...
LOG_FILE = ROOT / "artifacts" / "auth_smoke_minimal.log"
...
def main():
    ...
    for email, password, label in LOGINS:
        try:
            ...
            status = 0
            body = {"detail": str(e)}
        log("%s login status=%s body=%s" % (label, status, body))
